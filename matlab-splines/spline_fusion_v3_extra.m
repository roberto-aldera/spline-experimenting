...
T = {T0; T0; T0; T0; T0; T0; T0}; % pretending the transforms are all the same for now
...
control_pose_times = linspace(control_pose_delta_time,size(T,1)*control_pose_delta_time, ... 
...
spline_times = linspace(control_pose_delta_time,(size(T,1)-1)*control_pose_delta_time, ... 
...
% TODO - fix the timestamps! Annoying drop for each spline starting one
% And also not correct for every 0.25 s
...
legend('Control pose estimates','Spline estimates');
